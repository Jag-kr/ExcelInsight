  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
        <Merge className="h-4 w-4 text-primary" /> Merge Columns
      </h3>

      <div className="grid grid-cols-2 gap-3">
        <Select value={col1} onValueChange={setCol1}>
          <SelectTrigger className="bg-secondary border-border"><SelectValue placeholder="Column 1" /></SelectTrigger>
        ...
        <Select value={col2} onValueChange={setCol2}>
          <SelectTrigger className="bg-secondary border-border"><SelectValue placeholder="Column 2" /></SelectTrigger>
          <SelectContent>{columns.map(c => <SelectItem key={c.name} value={c.name}>{c.name}</SelectItem>)}</SelectContent>
        </Select>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <Input placeholder="New column name" value={newName} onChange={e => setNewName(e.target.value)} className="bg-secondary border-border" />
        <Input placeholder="Separator" value={separator} onChange={e => setSeparator(e.target.value)} className="bg-secondary border-border" />
      </div>
